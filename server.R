library(shiny)
library(tidyr)
library(dplyr)
library(ggplot2)
library(plotly)
library(cluster)

shinyServer(function(input, output) {
     
     datasetInput <- reactive({
          reporte <- reporte.final()
          if(is.null(reporte)) return(NULL)
          
          return(reporte)
     })
     
     output$download <- downloadHandler(
          
          filename = function() { paste("Asigned models by line", '.csv', sep='') },
          content = function(file) {
               write.csv(datasetInput(), file)
          }
     )
     
     leer.archivo <- reactive({
          # input$file1 will be NULL initially. After the user selects
          # and uploads a file, it will be a data frame with 'name',
          # 'size', 'type', and 'datapath' columns. The 'datapath'
          # column will contain the local filenames where the data can
          # be found.
          
          
          inFile <- NULL
          inFile <- input$browse          
          #inFile$datapath <- "tiempos.csv"
          
          if (is.null(inFile)){
               #sin archivo seleccionado
               return(NULL)
          }
          
          #leer por tipo de archivo
          datos <- read.csv(inFile$datapath, 
                            header = input$header, na.strings = c("NA",""))
          #convertir NAS en cero
          datos[is.na(datos)]<-0
          names(datos)[1] <- "SKU"
          
          #limitar cantidad de modelos
          #datos<- datos[1:20,]
          
          return(datos)
          
     })  
     #TABLA DE ESTILOS ASIGNADOS
     output$tabla_asignacion <- DT::renderDataTable({
          reporte <- reporte.final()
          if(is.null(reporte.final)) return(NULL)
          
          DT::datatable(reporte, options = list(pageLength = 25))
     })
     
     #ANALISIS FINAL
     output$total.fam <- renderTable({
          reporte <- reporte.final()
          if(is.null(reporte)) {
               return(NULL)
          } else {
               totales <- reporte%>%
                    group_by(PROD.LINE)%>%
                    summarise("Styles by production line"= n())
          }
     })
     
     #tabla outliers
     output$outliers <- renderTable({
          reporte <- reporte.final()
          if(is.null(reporte)) {
               return(NULL)
          } else {
               
          }
     })
     
     output$mejora <- renderTable({
          reporte <- reporte.final()
          if(is.null(reporte)) {
               return(NULL)
          } else {
               #crea tabla de 3 columnas ESTILO, PUESTO, TIEMPO
               fin <- dim(reporte)[2]-1
               tabla.totales <- gather(reporte, "OPERATOR","UNITS",c(2:fin))
               final <-tabla.totales%>%
                    group_by(PROD.LINE, OPERATOR)%>%
                    summarise("Average" = ceiling(mean(UNITS, na.rm = T)),
                              "Deviation" = ceiling(sd(UNITS, na.rm = T)),
                              "Percentage" = round(Deviation/Average*100,2))
               final[is.na(final)] <- 0
               final%>%
                    group_by(OPERATOR)%>%
                    summarise("Average" = ceiling(mean(Average)), 
                              "Deviation" = ceiling(sd(Deviation)) , 
                              "Percentage" = round(Deviation/Average*100,2))
          }
          
     })
     
     output$desviaciones <- renderTable({
          reporte <- reporte.final()
          if(is.null(reporte)) {
               return(NULL)
          } else {
               #crea tabla de 3 columnas ESTILO, PUESTO, TIEMPO
               fin <- dim(reporte)[2]-1
               tabla.renglon <- gather(reporte, "OPERATOR","UNITS",c(2:fin))
               
               desviaciones <- tabla.renglon%>%
                    group_by(PROD.LINE, OPERATOR)%>%
                    summarise("Average" = ceiling(mean(UNITS)),
                              "Deviation" = round(sd(UNITS),2),
                              "Min" = min(UNITS) ,
                              "Max" = max(UNITS))
          }
     })
     
     output$grafico.final <- renderPlot({
          reporte <- reporte.final()
          if(is.null(reporte)) {
               return(NULL)
          } else {
               #crea tabla de 3 columnas ESTILO, PUESTO, TIEMPO
               fin <- dim(reporte)[2]-1
               tabla.renglon <- gather(reporte, "OPERATOR","UNITS",c(2:fin))
               
               # grafico de desviaciones por puesto
               ggplot(data = tabla.renglon, aes(OPERATOR, UNITS)) + 
                    geom_boxplot(col = "navy", outlier.colour = "red") + 
                    facet_wrap(~PROD.LINE, scales = "free", ncol = 2) +
                    xlab("Products") +
                    ylab("Units")  +
                    ggtitle("Deviation to produce one pair")+
                    theme(axis.text=element_text(size=10))
          }
     })
     
     
     reporte.final <- reactive({
          
          #HASTA QUE NO SE SELECCIONE UN ARCHIVO
          tabla.raw <- leer.archivo()
          if(is.null(tabla.raw)) return(NULL)
          
          arbol <- dendograma()
          if(is.null(arbol)) return(NULL)
          nolineas <- max(cutree(arbol, h=input$altura_cluster))
          
          set.seed(88)
          gr <- tabla.raw[c(2:dim(tabla.raw)[2])]
          k <- kmeans(gr, centers = nolineas ,nstart = 10, iter.max = 100)
          agrupacion.final <- cbind(tabla.raw, "PROD.LINE" = k$cluster)
          reporte <- agrupacion.final%>%select(PROD.LINE, SKU)%>%
               arrange(PROD.LINE)
          
          #agregar cluster a tabla por renglon
          merge(tabla.raw, reporte, by = "SKU")
          
     })
     
     #TABLA COMPLETA DE ESTILOS Y TIEMPOS
     output$tabla_completa <- DT::renderDataTable({
          tabla.raw <- leer.archivo()
          if(is.null(tabla.raw)) return(NULL)
          
          DT::datatable(tabla.raw, options = list(pageLength = 10))
     })
     
     #GRAFICO DE CLUSTERS
     dendograma <- reactive({
          
          #leer tabla
          tabla.raw <- leer.archivo()
          if(is.null(tabla.raw)) return(NULL)
          
          set.seed(8)
          distancias <- dist(tabla.raw[,-1], method = "euclidian")
          clust <- hclust(distancias)
          return(clust)
     })
     
     #grafica el dendograma
     output$dendograma <- renderPlot({
          arbol <- dendograma()
          if(is.null(arbol)) return(NULL)
          g <- plot(arbol, axes = T, main = "Grouping cluster", 
                    xlab = "Modelos", ylab = "Deviation index")
          return(g + abline(h = input$altura_cluster, col="red"))
     })
     
     #escribe el n?mero de lineas que se crean en cada altura
     output$lineas <- renderText({
          arbol <- dendograma()
          if(is.null(arbol)) return(NULL)
          max(cutree(arbol, h=input$altura_cluster))
     })
     
     #TABLA INICIAL DE PROMEDIOS, DESVIACIONES Y RANGOS
     output$tablainicial <- renderTable({
          
          #leer tabla
          tabla.raw <- leer.archivo()
          if(is.null(tabla.raw)) return(NULL)
          
          #crea tabla de 3 columnas ESTILO, PUESTO, TIEMPO
          tabla.ren <- gather(tabla.raw, "OPERATOR","UNITS",-1)
          
          data.frame(tabla.ren%>%
                          group_by(OPERATOR)%>%
                          summarise("Average" = ceiling(mean(UNITS)),
                                    "Deviation" = round(sd(UNITS),2),
                                    "Min" = min(UNITS) ,
                                    "Max" = max(UNITS),
                                    "Percentage" = ceiling(Deviation/Average*100)))
          
     })
     
     #GRAFICO INICIAL
     output$graficoinicial <- renderPlot({
          
          #leer tabla
          tabla.raw <- leer.archivo()
          if(is.null(tabla.raw)) return(NULL)
          
          #crea tabla de 3 columnas ESTILO, PUESTO, TIEMPO
          tabla.ren <- gather(tabla.raw, "OPERATOR","UNITS",-1)
          
          # grafico de desviaciones por puesto
          ggplot(data = tabla.ren, aes(SKU, UNITS)) + 
               geom_point(col = "navy", alpha = 0.5) + 
               facet_wrap(~OPERATOR, scales = "free")+ 
               geom_hline(data = tabla.ren%>%
                               group_by(OPERATOR)%>%
                               summarise("Average" = mean(UNITS)),
                          aes(yintercept = Average), 
                          col = "red", lwd = 1) + 
               xlab("SKU") +
               ylab("Units by pair")  +
               ggtitle("Units to produce one pair")
          
     })
     #GRAFICO INICIAL
     output$boxplotini <- renderPlot({
          
          #leer tabla
          tabla.raw <- leer.archivo()
          if(is.null(tabla.raw)) return(NULL)
          
          #crea tabla de 3 columnas ESTILO, PUESTO, TIEMPO
          tabla.ren <- gather(tabla.raw, "OPERATOR","UNITS",-1)
          
          # grafico de desviaciones por puesto
          ggplot(data = tabla.ren, aes(OPERATOR, UNITS)) + 
               geom_boxplot(col = "navy") + 
               xlab("SKU") +
               ylab("Units by pair")  +
               ggtitle("Units to produce one pair")
          
     })
     
})
